/*
 * Trabalho 2 PCD - Ex 2 - Somatórias, seção crítica e reduções em OpenMP (utilizando critical)
...
int countAlive(double** grid, int* time_elapsed) {
    ...
    #pragma omp parallel firstprivate(totalLocal) private(i, j)
    {
        #pragma omp for
        for (i = 0; i < N; i++) {
            for (j = 0; j < N; j++) {
